// This Arduino sketch is used to control a minicircuit USB / TTL RF SP16T Switch, a 16 channel multiplexer. The parameter are set 
// through serial comunication between the arduino and a Raspberry Pi with EPICS, the ASYNC and StreamRecord libraries installed. 
...
// -- AUTO: this mode lets you select one or more channels of the multiplexer. The arduino will continuously switch between them, 
//          after it detects a trigger signal. you can set how long a channel is selected as output (dwell time), how many times 
//          you want to switch between the selected channels (cycles), and the delay from the trigger signal (global_delay).
...
// declaration of the global variables that will be used as parameters

int mode_select=1; // manual mode -> 1 automatic mode -> 0
long int dwell_time=100; // time to wait before switching to another channel in automatic mode
...
int mask=0b1; // binary number 1 used to scan the cahnnel_mask in auto mode
        ...
        { // the end of the command has arrived
            switch (input[0]) 
            {
                ...
                case 'D':// dwell time has come in
                    ...
                    sprintf(error, "dwell_changed_to_%d_us\n", dwell_time); // write to the error string what changed
